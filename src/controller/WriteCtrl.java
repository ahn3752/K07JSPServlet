...
public class WriteCtrl extends HttpServlet{
	...
	/*
	 글쓰기 페이지로 진입했을때 요청을 처리한다. 글쓰기 폼으로 이동(location)하는것은 get방식이므로
	 doGet()에서 처리한다.
	 */
	@Override
	...
	/*
	 글 작성을 완료한 후 서버로 전송(submit)할떄의 요청을 처리한다. 게시판에 글을 쓸때는 post방식으로 처리하게 된다. 
	 */
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(mr != null) {
			/*
			 파일 업로드가 완료되면 나머지 폼값을 받기위해 mr참조변수를 이용한다.			 
			 */
