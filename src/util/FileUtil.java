...
public class FileUtil {
	...
	public static MultipartRequest upload(
		...
		/*
		 매개변수 1 : 사용자의 요청정보가 저장된 request 내장객체
		 매개변수 2 : 파일이 저장된 서버의 물리적 경로 
		  	그 외 업로드할 파일의 최대용량, 인코딩방식 , 파일명 중복처리를 위한 클래스를 통해
		  	MultipartRequest객체를 생성한다.
		  	객체 생성이 완료되면 파일은 업로드 된다.
	...
	//파일 다운로드 처리 1
		...
		/*
		 파일 다운로드 처리2 : 원본파일명과 저장된 파일명을 지정하여 파일 다운로드시 파일명을 원본파일명으로 변경한다.
	...
	public static void download(
		...
		*/
		...
		try{
			//파일명이 한글인 경우 깨짐현상이 발생하므로 반드시 인코딩 해야한다.
			...
			*/
			...
			/*
			 서버에 저장된 파일명을 통해 파일객체를 생성한 후 파일을 다운로드 받을때
			 아래 설정된 응답헤더를 통해 파일명을 원본파일명으로 변경되어 로컬로 다운로드 된다. 
