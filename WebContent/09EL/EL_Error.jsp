...
<body>
	...
	<fieldset>
		...
		<!-- 
			문자열 형태로 숫자가 전달되면 연산시 자동으로 숫자로 변경된 후 연산된다.
			숫자로 변경할 수 없는 문자가 포함된다면 예외가 발생한다.
		...
		<!--
			EL에서 속성에 접근시 -이 포함된 경우에는 .으로 표현할 수 없고 
			반드시 [를 통해 표현해야 한다.
		...
		<h4>배열</h4>
		<!--
			JSTL의 forEach태그를 통해 반복출력한다. 해당 문장은 배열이나
			컬렉션의 크기만큼 반복하는 확장for문 형태로 기술되었다.
		  -->
		...
		<!--
			인덱스를 초과하면 예외가 발생되어야 하지만 EL에서는 기본적인 예외를
			잡아주므로 에러가 발생되지는 않는다.
		  -->
		<h2>인덱스를 벗어난 경우</h2>
		<ul>
		...
		<ul>
			...
			<li>속성은 존재, getter가 없는경우 : \${eldto.id }
				:
				\${eldto.id } [500에러발생]
				</li>
