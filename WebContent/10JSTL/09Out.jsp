...
<body>
	...
	<!-- 
	-out태그에 value속성의 값이 null일때는 default속성에 지정된 값이 출력된다.
	-단 , 값이 빈문자열("")인 경우는 값이 있는것으로 간주되어 default속성에
	지정된 값이 출력되지 않는다.
	 -->
	<h3>default속성</h3>
	
	<h4>값이 빈 문자열 : 값이 있는 경우에 해당</h4>
	츨력 : <c:out value="" default="값이 빈 문자열" />
	...
	있는 경우에는 해당 파라미터 값을 가져와서 페이지에 적용한다.
	 -->
	<h3>페이지 링크에 응용하기</h3>
	<c:url value="/08Board1/BoardList.jsp?nowPage=" /><c:out value="${param.nowPage }" default="1" />
	<hr/>
	<!-- 주의]한줄로 붙여서 작성하세요. 줄바꿈하면 공백이 들어갑니다.  -->
	<a href="<c:url value="/08Board1/BoardList.jsp?nowPage=" /><c:out value="${param.nowPage }" default="1" />">회원제게시판 리스트 바로가기</a>
