...
<body>
	...
	-else문이 없기 때문에 다중조건일때는 표현이 어렵다. 이떄는
	주로 choose문을 사용한다.
	형식]
		test : 조건
	...
	<!-- 
		if(numVar%2==0) 와 동일한 조건으로, 해당 조건문은 true를 
		반환하므로 태그사이의 문장이 출력된다. 그리고 var속성에 지정된
		result에 true가 저장된다.
	  -->
	...
	<!-- 
		JSTL의 if문은 else가 없기 때문에 참 or 거짓의 형태로 구문을 만들때는
		아래와 같이 참의 조건, 거짓의 조건으로 각각 블럭을 만들어 준다.
	 -->
...
<!-- 
	...
	<c:if test="${not empty param.user }">
      ...
      <!--  
      첫 번째 if문에서의 결과에 따라 두 번째 if문은 반대의 결과를
      가지게 되는 구조로 작성되었다. 즉 loginResult가 true가 된다면
      두 번째 if문은 false값으로 바뀌게 된다.
      -->
