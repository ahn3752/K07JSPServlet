...
여러 개의 파라미터를 한꺼번에 저장한 후 DAO의 메소드를 호출할 때 전달한다.
...
if (searchWord != null) {
   /*
   리스트 페이지에 최초 진입 시에는 파라미터가 없으므로 if로 구분하여
   파라미터가 있을 때만 Map에 추가되도록 한다.
   */
...
//int totalRecordCount = dao.getTotalRecordCount(param);//joinX
int totalRecordCount = dao.getTotalRecordCountSearch(param);//joinO
...
/*
전체 페이지 수 계산 => 게시물이 108개라 가정했을 때 
                  108/10 => 10.8 => ceil(10.8) => 11페이지
*/
...
//한 페이지에 출력할 게시물의 범위를 결정한다. 계산식은 교안을 참조한다.(1~10, 11~20, ...)
                     ...
                     */
                     for (BbsDTO dto : bbs) {
                        ...
                        */
                        ...
                        /*
                        전체게시물 수 : 108개
                        페이지사이즈(web.xml에서 PAGE_SIZE로 설정) : 10
                        현재 페이지 1일 때
                     ...
                     <tr>
                        ...
                        <td class="text-left"><a href="BoardView.jsp?num=<%=dto.getNum()%>&nowPage=<%=nowPage%>&<%=queryStr%>"><%=dto.getTitle()%>
                        </a></td>
